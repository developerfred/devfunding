	...
	useEffect(() => {
		const fetchGovernanceData = async () => {
			try {
				...
				const proposalCount =
					await contractInteractions.viewFunctions.getProposalCount();

				// Fetch all proposals
				const fetchedProposals = [];
				for (let i = 0; i < Number(proposalCount); i++) {
					const proposal =
						await contractInteractions.viewFunctions.getProposal(i);
					fetchedProposals.push(proposal);
				}

				// Fetch votes for each proposal
				const fetchedVotes = await Promise.all(
					fetchedProposals.map((_, index) =>
						contractInteractions.viewFunctions.getVotes(index),
					),
				);
